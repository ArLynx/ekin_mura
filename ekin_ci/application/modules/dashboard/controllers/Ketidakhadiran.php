...
class Ketidakhadiran extends MY_Controller
{
    ...
    public function get_data()
    {
        ...
        if (!empty($unor) && !empty($day) && !empty($month) && !empty($year)) {
    ...
    {
        $data['page_title'] = 'Ketidakhadiran';
        $data['breadcrumb'] = [
            ['link' => '', 'title' => 'ketidakhadiran', 'icon' => '', 'active' => '1'],
        ];
        $data['active_menu'] = 'ketidakhadiran';
        ...
        $this->render('ketidakhadiran/index', $data);
    }
    ...
    public function add()
    {
        if ($this->input->post()) {
            ...
            if (!empty($keterangan) && !empty($uraian)) {
                $code      = $PNS_PNSNIP;
                $save_data = array(
                    'code'       => $code,
    ...
    {
        if ($this->input->post()) {
            $id_absen_enroll_ket = $this->input->post('id_absen_enroll_ket', true);
            $id_pns              = $this->input->post('id_pns', true);
            $PNS_PNSNIP          = $this->input->post('PNS_PNSNIP', true);
            $PNS_PNSNAM          = $this->input->post('PNS_PNSNAM', true);
            $PNS_GLRDPN          = $this->input->post('PNS_GLRDPN', true);
            ...
            $keterangan          = $this->input->post('keterangan', true);
            $uraian              = $this->input->post('uraian', true);
            if (!empty($id_absen_enroll_ket) && !empty($keterangan) && !empty($uraian)) {
                $code      = $PNS_PNSNIP;
                $edit_data = array(
                    'code'       => $code,
                    'PNS_PNSNIP' => $PNS_PNSNIP,
                ...
                $this->absen_enroll_model->edit($id_absen_enroll_ket, $edit_data);
    ...
    public function delete($id_absen_enroll = null)
    {
        if (!is_null($id_absen_enroll)) {
            ...
            if ($check_exists) {
                $check_mutasi_detail_pending = $this->mutasi_detail_model->first([
                    'pns_pnsnip' => $check_exists->PNS_PNSNIP,
                    'status'     => '0',
                ]);

                $check_unor = [$check_exists->PNS_UNOR];

                if ($check_mutasi_detail_pending) {
                    $check_unor[] = $check_mutasi_detail_pending->pns_unor_baru;
                }

                if (in_array(get_session('unor'), $check_unor) || get_session('id_groups') == 5) {
