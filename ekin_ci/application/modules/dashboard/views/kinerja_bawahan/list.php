...
<section class="content" data-id_groups="<?php echo get_session('id_groups'); ?>" data-updated="<?php echo $_updated; ?>" data-deleted="<?php echo $_deleted; ?>">
	...
	<div class="box">
		...
		<div class="box-body">
			<b>Batas akhir</b> Penilaian PNS adalah <b>Tanggal <?php echo $this->kegiatan_model->get_tgl_max_approve()->row()->bulan; ?> Pukul 23:59</b> pada bulan berikutnya.
		</div>
		...
		<div class="box-body" style="padding-top: 0;">
			<div class="row">
				<div class="col-md-12">
					...
					<!-- base_url + '/dashboard/kinerja_bawahan/get_data' -->
					<form id="fromTanggapanKeg" action="<?php echo base_url('/dashboard/kinerja_bawahan/tanggapan-all-kegiatan'); ?>" method="post">
						...
						<table id="datatableKegiatanBwh" class="table table-striped table-bordered" style="width: 100%;">
							<thead>
								<tr>
									...
									<!-- <th width="20" >
										<a class='btn btn-success' href='javascript:void(0)' style="width:0px; height:26px" title="Setuju Semua" onclick="simpan_semua_kegiatan(1);"><i class="fa fa-check-circle" style="position: absolute; margin:-0px 0px 0px -6px;"></i></a><br />
										<a class='btn btn-danger' href='javascript:void(0)' style="width:0px; height:26px" title="Tolak Semua" onclick="simpan_semua_kegiatan(4);"><i class="fa fa-times-circle" style="position: absolute; margin:-0px 0px 0px -6px;"></i></a><br /><input style="margin-left: 7px;" type="checkbox" id="select_all" />
									</th> -->
								</tr>
							</thead>
						</table>
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Konfirmasi LKH Bawahan</h4>
            </div>
            <div class="modal-body">
                <div class="card">
                    ...
                    <div class="card-footer">
						<button class="btn btn-sm btn-primary save-edit-setujui">
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Konfirmasi LKH Bawahan (Koreksi)</h4>
            </div>
            <div class="modal-body">
                <div class="card">
                    <div class="card-body">
                    ...
                    <div class="card-footer">
						<button class="btn btn-sm btn-primary save-edit-koreksi">
                            <i class="fa fa-dot-circle-o"></i> Simpan</button>
                    </div>
                </div>
            </div>
...
<script>
	var datatable = $('#datatableKegiatanBwh').DataTable({
		"columns": [{
				"width": "1"
			},
			{
	...
	function getData() {
        ...
        $.get(base_url + '/dashboard/kinerja_bawahan/get_data', {
			...
			.then(function (response) {
				...
				$.each(response, function (key, value) {
					...
					// arrAksiCek += '<div class="btn-group btn-group-sm" role="group" aria-label="">' +
					// 				'<input type="checkbox" name="idnya[]" class="checkbox" value="\'' +value.id_encrypt + '\'" style="position: absolute; margin:-0px 0px 0px 7px;">' +
					//               '</div>';
		...
		$.get(base_url + '/api/get_kinerja_bawahan_setujui_byid', {
            ...
            .then(function(response) {
                $.each(response, function(key, value) {
					...
					// $(value).html($(value).data('reset-text'));
                });
            })
	...
	$(".save-edit-setujui").click(function() {
        ...
        $.get(base_url + '/api/save_edit_setujui', {
			...
			.then(function(response) {
				// $.each(response, function(key, value) {
				// 	let norma_waktu_max = value.norma_wkt;
				// 	console.log("norma max : " + norma_waktu_max);
				// });
		...
		$.get(base_url + '/api/get_kinerja_bawahan_koreksi_byid', {
            ...
            .then(function(response) {
                $.each(response, function(key, value) {
					...
					$(".textkomentarkoreksi").val(value.komentar_atasan);
	...
	$(".save-edit-koreksi").click(function() {
		let komentar_atasan = $("#textkomentarkoreksi").val();
		let id_kegiatan = $("#idkegiataneditkoreksi").val();
		
        $.get(base_url + '/api/save_edit_koreksi', {
			id_kegiatan: id_kegiatan,
			...
			.then(function(response) {
				if(komentar_atasan == ""){
					Swal.fire({
						type: 'error',
						title: 'Oops...',
						text: 'Komentar atasan tidak boleh kosong'
					});
				}else{
					Swal.fire({
						type: 'success',
	...
	var table = $('#datatableKegiatanBwh').DataTable();
