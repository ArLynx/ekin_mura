	...
	<div class="box box-primary">
		...
		<div class="box-body">
			...
			<div class="alert alert-info alert-dismissible fade in text-center" role="alert">
				<p class="text-danger">Aplikasi ini mencatat apa yang kita tuliskan, tetapi <b><?php if($pns->PNS_AGAMA == 1)  echo "Allah SWT"; else echo "Tuhan YME"; ?></b> tahu apa yang kita lakukan.</p>
			...
			<div class="row">
				...
				<div class="col-md-2">
					<div class="form-group md-form md-outline <?php echo form_error('jam_input') ? 'has-error' : ''; ?>">
						<label for="jam_input">Jam Mulai</label>
						<input type="time" name="jam_input" id="jam_input" class="form-control" placeholder="Jam" style="width:110px;"
				...
				<div class="col-md-2">
					<div class="form-group <?php echo form_error('durasi_input') ? 'has-error' : ''; ?>">
						<label for="durasi_input" style="margin-left:-80px;">Durasi (Menit)</label>
						<input type="text" name="durasi_input" id="durasi_input" class="form-control" placeholder="Durasi" style="margin-left:-80px; width:65px;"
			...
			<div class="form-group <?php echo form_error('analisis_tugas') ? 'has-error' : ''; ?>">
				...
				<input type="hidden" name="analisis_tugas_temp" id="analisis_tugas_temp" value="<?php echo isset($kegiatan) ? $kegiatan->rincian_pekerjaan_id : (set_value('analisis_tugas_temp') == null ? '' : set_value('analisis_tugas_temp')); ?>" />
			...
			<div class="form-group <?php echo form_error('komentar_atasan') ? 'has-error' : ''; ?>">
				<label for="komentar_atasan">Komentar <?php if($kegiatan->status == 2 || $kegiatan->status == 3)  echo "Atasan"; else echo "Penilai"; ?></label>
				<textarea name="komentar_atasan" id="komentar_atasan" class="form-control" placeholder="Komentar Atasan" disabled="disabled"
					cols="30"
					rows="4"><?php echo isset($kegiatan) ? ($kegiatan->status == 2 || $kegiatan->status == 3 ? $kegiatan->komentar_atasan : ($kegiatan->status == 7 || $kegiatan->status == 8 ? $kegiatan->komentar_penilai : set_value('komentar_penilai'))) : set_value('komentar_atasan'); ?></textarea>
				<?php echo form_error('komentar_atasan', '<p class="help-block text-red">', '</p>'); ?>
				<input type="hidden" name="status" id="status" value="<?php echo isset($kegiatan) ? $kegiatan->status : set_value('status'); ?>" />
			</div>
