		...
		<div class="box-body">
			...
			<div class="form-group <?php echo form_error('nominal') ? 'has-error' : ''; ?>">
				...
				<input type="text" name="nominal" class="form-control input-currency" placeholder="Nominal"
					value="<?php echo isset($nominal_sanksi) ? $nominal_sanksi->nominal : set_value('nominal'); ?>">
				<?php echo form_error('nominal', '<p class="help-block text-red">', '</p>'); ?>
			</div>

			<div class="form-group <?php echo form_error('mulai_tanggal') ? 'has-error' : ''; ?>">
				<label for="mulai_tanggal">Mulai Tanggal</label>
				<input type="date" name="mulai_tanggal" class="form-control" placeholder="Mulai Tanggal"
					value="<?php echo isset($nominal_sanksi) ? $nominal_sanksi->mulai_tanggal : set_value('mulai_tanggal'); ?>">
				<?php echo form_error('mulai_tanggal', '<p class="help-block text-red">', '</p>'); ?>
			</div>

			<div class="form-group <?php echo form_error('sampai_tanggal') ? 'has-error' : ''; ?>">
				<label for="sampai_tanggal">Sampai Tanggal</label>
				<input type="date" name="sampai_tanggal" class="form-control" placeholder="Sampai Tanggal"
					value="<?php echo isset($nominal_sanksi) ? $nominal_sanksi->sampai_tanggal : set_value('sampai_tanggal'); ?>">
				<?php echo form_error('sampai_tanggal', '<p class="help-block text-red">', '</p>'); ?>
			</div>

			<div class="form-group <?php echo form_error('keterangan') ? 'has-error' : ''; ?>">
				<label for="keterangan">Keterangan Sanksi</label>
				<textarea name="keterangan" id="keterangan" class="form-control" placeholder="Keterangan Sanksi"
					cols="30"
					rows="10"><?php echo isset($nominal_sanksi) ? $nominal_sanksi->keterangan : set_value('keterangan'); ?></textarea>
