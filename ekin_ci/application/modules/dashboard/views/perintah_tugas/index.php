<!-- Main content
# @Author: Awan Tengah
# @Date:   2019-08-06T08:44:56+07:00
# @Last modified by:   Awan Tengah
# @Last modified time: 2019-09-03T18:49:31+07:00
-->

<section class="content" ng-app="perintahTugasModule" ng-controller="perintahTugasController as mc">
        ...
        <div class="box-header">
            <div class="row" ng-cloak>
                <div class="col-md-6" ng-if="<?php echo (isset($_created) == 1); ?>">
                    <a href="<?php echo site_url('dashboard/perintah-tugas/add'); ?>" class="btn btn-primary">Tambah</a>
                </div>
                <div class="col-md-6">
                ...
                <table class="table table-hover table-bordered margin-0" st-pipe="mc.callServer" st-table="mc.displayed" st-safe-src="mc.callServer" refresh-table>
                    <thead>
                        <tr>
                            ...
                            <th>Perihal</th>
                            <th width="200">Lama Penugasan</th>
                    ...
                    <tbody ng-show="!mc.isLoading" ng-cloak>
                        <tr ng-repeat="row in mc.displayed">
                            ...
                            <td>{{row.no_surat}}</td>
                            <td>{{row.keperluan}}</td>
                            <td>{{row.lama_penugasan}}</td>
                            <td class="td-action">
                                <div class="btn-group btn-group-md" role="group" aria-label="...">
                                    <button type="button" class="btn btn-success" ng-click="openDetailModal(row)" title="Detail">
                                        <i class="ion-eye"></i>
                                    </button>
                                    <a href="<?php echo site_url('dashboard/perintah-tugas/edit/{{row.id}}'); ?>" ng-if="<?php echo (isset($_updated) == 1); ?>" class="btn btn-warning" title="Ubah">
...
<script>
    var app = angular.module('perintahTugasModule', ['smart-table', 'oitozero.ngSweetAlert', 'ui.bootstrap']);
    ...
    app.factory('Resource', ['$q', '$filter', '$timeout', '$http', function($q, $filter, $timeout, $http) {
        ...
        function getPage(start, number, params) {
            ...
            var url = base_url + '/dashboard/perintah-tugas/get-data';
    ...
    app.controller('perintahTugasController', ['Resource', '$scope', 'SweetAlert', '$http', '$uibModal', '$timeout',
                ...
                SweetAlert.swal({
                    ...
                    function(isConfirm) {
                        if (isConfirm) {
                            ...
                            if (index !== -1) {
                                $http.delete("<?php echo base_url('dashboard/perintah-tugas/delete'); ?>" + "/" + row.id);
