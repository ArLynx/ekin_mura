<link rel="stylesheet" href="<?php echo base_url('assets/plugin/sweetalert2/dist/sweetalert2.min.css'); ?>">
<style>
	.swal2-popup {
		font-size: 1.6rem !important;
	}

</style>
<script src="<?php echo base_url('assets/plugin/sweetalert2/dist/sweetalert2.min.js'); ?>"></script>

<!-- Main content -->
<section class="content" data-id_groups="<?php echo get_session('id_groups'); ?>" data-updated="<?php echo $_updated; ?>" data-deleted="<?php echo $_deleted; ?>">

	<!-- Your Page Content Here -->
	<div class="box">
		<div class="box-header with-border">
			<div class="row">
				<div class="col-md-4">
					<div class="form-group">
						<select class="form-control select2" name="selected_sopd" onchange="getData()">
							<?php if (get_session('id_groups') == '1' || get_session('id_groups') == '5'): ?>
							<option value="">- Pilih SOPD -</option>
							<?php endif;?>
							<?php if ($all_sopd): ?>
							<?php foreach ($all_sopd as $row): ?>
							<option value="<?php echo encode_crypt($row->KD_UNOR); ?>"><?php echo $row->NM_UNOR; ?>
							</option>
							<?php endforeach;?>
							<?php endif;?>
						</select>
					</div>
				</div>
			</div>
		</div>
		<div class="box-body" style="padding-top: 0;">
			<div class="row">
				<div class="col-md-12">
					<div class="table-responsive">
						<?php alert_message_dashboard();?>
						<table id="datatableMKJ" class="table table-striped table-bordered" style="width: 100%;">
							<thead>
								<tr>
									<th>No</th>
									<th>Kategori PNS</th>
									<th>Kelas Jabatan</th>
									<th>Nama Jabatan</th>
									<th>Unit Organisasi</th>
									<th>Nama PNS</th>
								</tr>
							</thead>
						</table>
					</div>
				</div>
			</div>
		</div>
	</div>

</section>
<!-- /.content -->

<script>
	var datatable = $('#datatableMKJ').DataTable({
		"columns": [{
				"width": "10"
			},
			null,
			{
				"width": "90"
			},
			null,
			null,
			{
				"width": "250"
			},
			
		],
		"aaSorting": [],
	});

	$("a#addLinkKJ").hide();

	let _updated = $(".content").attr('data-updated');
	let _deleted = $(".content").attr('data-deleted');
	let _id_groups = $(".content").attr('data-id_groups');

	$(function () {
		if(_id_groups != '1' && _id_groups != '5') {
			getData();
		}
	});

	function getData() {
		let selected_sopd = $("select[name=selected_sopd]").val();
		datatable.clear().draw();

		$("#datatableMKJ > tbody").html(`
			<tr>
				<td colspan="7" class="text-center">
					<img src="`+base_url +`assets/img/loading.svg">
				</td>
			</tr>
		`);

		$.get(base_url + '/dashboard/peta_jabatan/get_data', {
				selected_sopd: selected_sopd
			})
			.then(function (response) {
				$("a#addLinkKJ").attr('href', base_url + '/dashboard/master-kelas-jabatan/add/' + selected_sopd);
				$("a#addLinkKJ").show();
				if(response != '') {
					let arrData = [];
					$.each(response, function (key, value) {
						let arrAksi = '';
						let data = [
							++key,
							(value.jabatan_pns != null ? value.jabatan_pns : '-'),
							value.kelas_jabatan,
							value.nama_jabatan,
							value.unit_organisasi,
						
						];
						// if (_updated == 1) {
				
							data.push('<div class="text">' +
								'<div class="btn-group btn-group-md" role="group" aria-label="...">' +
								'<p class="text-bold" >'+ (value.nama_pns? (value.gelar_depan? value.gelar_depan : ' ') + ' ' +  value.nama_pns + ' ' + (value.gelar_belakang? value.gelar_belakang : ' ') : '') +'</i></p>' 
								+ '<p class="text" >'+ (value.nip? value.nip : '') + '</i></p>' +
								'</div>' +
								'</div>');
						
						arrData.push(data);
					});
					datatable.rows.add(arrData).draw(false);
				} else {
					$("#datatableMKJ > tbody").html(`
						<tr>
							<td colspan="7" class="text-center">
								No data available in table
							</td>
						</tr>
					`);
				}
			});
	}

	function deleteData(id_encrypt) {
		const swalWithBootstrapButtons = Swal.mixin({
			customClass: {
				confirmButton: 'btn btn-success',
				cancelButton: 'btn btn-danger'
			},
			buttonsStyling: false
		})

		swalWithBootstrapButtons.fire({
			title: 'Apakah Anda yakin ingin menghapus data ini?',
			text: "",
			type: 'warning',
			showCancelButton: true,
			confirmButtonText: 'Hapus',
			cancelButtonText: 'Batal'
		}).then((result) => {
			if (result.value) {
				$.get(base_url + "/dashboard/peta_jabatan/delete", {
					id_encrypt: id_encrypt
				}).then(function () {
					getData();
					swalWithBootstrapButtons.fire(
						'Hapus berhasil',
						'Data berhasil dihapus',
						'success'
					)
				});
			} else if (
				/* Read more about handling dismissals below */
				result.dismiss === Swal.DismissReason.cancel
			) {
				swalWithBootstrapButtons.fire(
					'Hapus dibatalkan',
					'Data aman',
					'error'
				)
			}
		})
	}

</script>
