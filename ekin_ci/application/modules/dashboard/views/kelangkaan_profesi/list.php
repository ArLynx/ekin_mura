                 		...
                 		<div class="form-group">
							<select class="form-control select2" id="selected_year" name="selected_year" onchange="getDataYear(this.val)"
                                    ...
                                    <!-- <?php if($selected_year): ?>
                                         <option value="<?= $selected_year ?>" selected><?php echo $selected_year ?></option>
                                        <?php endif;?> -->
				...
				<div class="col-md-12">
					<div class="table-responsive">
						<form id="fromTanggapanVer" action="#" method="post">
							...
							<table id="datatablekelangkaanprofesi" class="table table-striped table-bordered" style="width: 100%;">
								<thead>
									<tr>
										<th rowspan="1" class="text-center th-top">No<z/th>
										<th rowspan="1" class="text-center th-top">Jabatan</th>
...
<div class="modal fade" id="myModal" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
				  ...
				  <h3 class="modal-title" id="exampleModalLabel" class='text-bold'>Input Kelangkaan Profesi</h3>
            </div>
            <div class="modal-body">
        <?php echo form_open('dashboard/kelangkaan_profesi/add'); ?>
            <div class="form-group">
    ...
    <div class="form-group">
        ...
        <label for="besaran_tpp">Nilai:</label>
         <input type="hidden" class="form-control" id="kelangkaan_id" name="kelangkaan_id" placeholder="Masukkan Nilai">
...
<div class="modal fade" id="deleteModal" role="dialog" aria-labelledby="deleteModal" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
        <?php echo form_open('dashboard/kelangkaan_profesi/delete'); ?>
        <input type="hidden" id="kelangkaan_id_delete" name="kelangkaan_id_delete">
...
<script>
 ...
 function getDataYear() {
    ...
    $.get(base_url + '/api/get_kelangkaan_profesi', {
        selected_year: selected_year
    })
    .then(function (response) {
			console.log(response)
      		let datatable = $("#datatablekelangkaanprofesi").DataTable({
            destroy: true,
            aaSorting: [],
            bSort: false,
            ...
            columns: [
                ...
                { data: "nama_jabatan",render: function(data, type, row) { return '<h6 class="text-bold">'+data+'</h6>' } },
				...
				{ data: 'id', render: function (data, type, row, meta) { return '  <div class="text-center"><button type="button"  onclick="editModal('+data + ','+ row.unor + ','+ row.tahun + ',' + row.besaran_tpp + ',' + row.kelangkaan_id + ')" class="btn btn-default" style="padding: 8px 12px 6px;" ><i class="fa fa-edit"></i></button> <button type="button"  class="btn btn-danger" onclick="deleteModal('+row.kelangkaan_id +')" style="padding: 8px 12px 6px;" ><i class="fa fa-trash"></i></button> </div>' } },
            ],
...
function getData(val) {
  ...
  $.get(base_url + '/dashboard/kelangkaan_profesi/get_data', {
...
function editModal(id, unor, tahun, besaran_tpp,kelangkaan_id){

    $('#edit').val(1);
    $('#kelangkaan_id').val(kelangkaan_id);
...
function deleteModal(kelangkaan_id){
    let selected_tahun = $('select[name=selected_tahun]').val();
    console.log(kelangkaan_id)
     $('#kelangkaan_id_delete').val(kelangkaan_id);
